...
export function SingleItemOptions(props: SingleItemOptions){
    ...
    const [selectedItems, setSelectedItems] = useState<Addition[]>(singleItem.additions);

    const handleSelect = (selectedChild: Addition | null) => {
      if (selectedChild) {
      ...
      <div>
        {items.options.map((item: any) => (
          <CategoryCarousel
            key={item.id}
            category={item}
            selectedValue={selectedItems.find(selectedItem => selectedItem.type === item.name)}
