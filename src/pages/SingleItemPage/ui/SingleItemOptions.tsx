...
export function SingleItemOptions(props: SingleItemOptions){
    ...
    const [selectedItems, setSelectedItems] = useState<MenuItemResponse['additions']>(singleItem.additions);


    const handleSelect = (selectedChild: MenuItemResponse['additions'][number] | null) => {
        if (selectedChild) {
          setSelectedItems([selectedChild]);
        } else {
          setSelectedItems([]);
        }
      };
        ...
        <div>
            {items.options.map((item) => (
                <CategoryCarousel
                    key={item.id}
                    category={item}
                    selectedValue={selectedItems.find((selectedItem) => selectedItem.id === item.id) || null}
